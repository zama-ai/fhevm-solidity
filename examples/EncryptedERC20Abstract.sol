...
abstract contract EncryptedERC20Abstract {
    ...
    constructor(
        ...
        uint8 decimals_,
        FHEVMConfigStruct memory FHEVMConfiguration
    ) {
        FHE.setCoprocessor(FHEVMConfiguration); // Set up the FHEVM configuration for this contract
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
    }
    ...
    function _mint(address account, uint64 mintedAmount) internal virtual {
        balances[account] = FHE.add(balances[account], mintedAmount); // overflow impossible because of next line
